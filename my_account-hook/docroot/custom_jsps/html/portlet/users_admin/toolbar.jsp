	...
	<c:if test="<%= hasAddOrganizationPermission || hasAddUserPermission %>">
		<liferay-ui:icon-menu align="left" cssClass='<%= "lfr-toolbar-button add-button " + (toolbarItem.equals("add") ? "current" : StringPool.BLANK) %>' direction="down" extended="<%= false %>" icon='<%= themeDisplay.getPathThemeImages() + "/common/add.png" %>' message="add" showWhenSingleIcon="<%= true %>">
			<c:if test="<%= hasAddUserPermission %>">
				...
				<portlet:renderURL var="addInternshipUser">
					<portlet:param name="struts_action" value="/users_admin/edit_user" />
					<portlet:param name="user_type" value="internship" />
					<portlet:param name="redirect" value="<%= viewUsersURL %>" />
				</portlet:renderURL>

				<liferay-ui:icon
					image="user_icon"
					message="Add Internship User"
					url="<%= addInternshipUser %>"
				/>

				<portlet:renderURL var="addScholarshipUser">
					<portlet:param name="struts_action" value="/users_admin/edit_user" />
					<portlet:param name="user_type" value="scholarship" />
					<portlet:param name="redirect" value="<%= viewUsersURL %>" />
				</portlet:renderURL>

				<liferay-ui:icon
					image="user_icon"
					message="Add Scholarship User"
					url="<%= addScholarshipUser %>"
				/>

				<portlet:renderURL var="addCIMUserURL">
					<portlet:param name="struts_action" value="/users_admin/edit_user" />
					<portlet:param name="user_type" value="cim" />
					<portlet:param name="redirect" value="<%= viewUsersURL %>" />
				</portlet:renderURL>

				<liferay-ui:icon
					image="user_icon"
					message="Add CIM User"
					url="<%= addCIMUserURL %>"
				/>
			</c:if>
