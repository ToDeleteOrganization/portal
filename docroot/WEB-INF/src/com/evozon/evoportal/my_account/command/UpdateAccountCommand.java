...
public class UpdateAccountCommand extends UserProfileAccountCommand {
	...
	private void executeExtraDaysEmailNotification(FreeDaysManager freeDaysManager) {
		try {
			List<User> allAccUsers = EvoportalUserUtil.getAccountingRegularUsers();
			List<User> allHrUsers = EvoportalUserUtil.getHRRegularUsers();
			List<User> allUsers = new ArrayList<User>();
			allUsers.add(freeDaysManager.getUser()); // affected user
			allUsers.addAll(allAccUsers);
			allUsers.addAll(allHrUsers);
